'''
  ...
  Here's the prompts (the full interaction) that led to this script.

  Given a list of images, it will show you a pair of images side by side.

  This is useful for displaying two images taken as a stereo photo in two separate shots. 

  If you took the view from the right, then the view from the left, and use this script, you can run this script without the "--reverse" option. It will show you the parallel view.

  If you took the view from the left, then the view from the right, then you should use the "--reverse" option. This will show the pairs in a crossed view. 
  ...
  Keys: 
     ...
     Note that you have to press the arrow keys twice to get to the next pair. 

  Yoonsuck Choe  choe@tamu.edu
  Sun, Jun 23, 2024 10:34:53 PM
'''
import glob
import argparse
...
class ImageViewer(tk.Tk):
    ...
    def display_images(self):
        ...
        left_image = Image.open(self.image_files[self.index])
        left_image = self.rotate_image_exif(left_image)
        left_image = self.scale_image(left_image, self.screen_width // 2, self.screen_height)
        ...
        if self.index + 1 < len(self.image_files):
            right_image = Image.open(self.image_files[self.index + 1])
            right_image = self.rotate_image_exif(right_image)
            right_image = self.scale_image(right_image, self.screen_width // 2, self.screen_height)
